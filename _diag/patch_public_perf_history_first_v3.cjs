...
const marker = "XS_FIX_FRONT_PERF_HISTORY_FIRST_NO_403_V3";

let text = fs.readFileSync(file, "utf8");

if (text.includes(marker)) {
  console.log("Patch V3 déjà présent, aucune modification.");
  process.exit(0);
}
...
if (braceStart < 0) {
  throw new Error("Impossible de trouver l'accolade ouvrante de publicPlayerPerformance");
}
...
if (end < 0) {
  throw new Error("Impossible de trouver la vraie fin de publicPlayerPerformance");
}
...
/* XS_FIX_FRONT_PERF_HISTORY_FIRST_NO_403_V3
 ...
 * Si history est vide, on retourne un fallback propre au lieu de casser l'UI.
 */
export async function publicPlayerPerformance(
  slug: string,
  opts?: { deviceId?: string; limit?: number } | string
  ...
  const empty: any = {
    ...
    meta: {
      ...
      marker: "XS_FIX_FRONT_PERF_HISTORY_FIRST_NO_403_V3",
  ...
  try {
    const historyUrl = \`${base}/history/player-chart/\${encodeURIComponent(s)}?limit=\${limit}\`;
